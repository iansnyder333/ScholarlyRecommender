...
def rank(k: int = 5):
    ...
    train = np.array([(row["Abstract"], row["Id"]) for _, row in likes.iterrows()])
    test = np.array([(row["Abstract"], row["Id"]) for _, row in candidates.iterrows()])
